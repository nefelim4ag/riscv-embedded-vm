    ...
    switch (opcode) {
        ...
        case 4:
            ...
            if (microop == 0x0) { // c.srli
                uint8_t rd8 = (inst >> 7) & 0x7;
                uint8_t shamt = (inst >> 2) & 0x1f;
                if (MODE)
                    evm_print("c.srli x[%d] = x[%d] >>u %d // %x -> %x\n",
                        8 + rd8, 8 + rd8, shamt, X8[rd8], X8[rd8] >> shamt);
                if (MODE == EVM_MODE_DISASM)
                    break;
                X8[rd8] = X8[rd8] >> shamt;
                ...
                if (mmop == 0x0) { // c.sub
                    ...
                    if (MODE == EVM_MODE_DISASM)
                        break;
                    X8[rd8] = X8[rd8] - X8[rs28];
                } else if (mmop == 0x1) { // c.xor
                    if (MODE)
                    ...
                    if (MODE == EVM_MODE_DISASM)
                        break;
                    X8[rd8] = X8[rd8] | X8[rs28];
                } else if (mmop == 0x3) { // c.and
                    if (MODE)
                        evm_print("c.and x[%d] = x[%d] & x[%d] // %x & %x\n",
                            8 + rd8, 8 + rd8, 8 + rs28, X8[rd8], X8[rs28]);
                    if (MODE == EVM_MODE_DISASM)
                        break;
                    X8[rd8] = X8[rd8] & X8[rs28];
                }
                ...
                uint8_t mode, struct evm_args *args)
{
    ...
    while (PC < end) {
        ...
        if (prefix == 0x3) {
            const uint8_t opcode = (*ptr >> 2) & 0x1f; // 7 bit
            if (MODE)
